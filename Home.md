---
type: hub
project: World of Spirits
status: active
---

# World of Spirits

> A 2D roguelike survival game about a Spirit Tamer restoring balance between the Human and Spirit Worlds.

## Start here

- [[Showcase/World of Spirits|Public showcase]]
- [[Notes/Game Design/Game Overview|Game overview]]
- [[Notes/Game Design/Spirits|Spirits and abilities]]
- [[Notes/Game Design/Areas and Bosses|Areas and bosses]]
- [[Notes/Development/Technical Overview|Technical overview]]
- [[Notes/Planning/Roadmap|Roadmap]]
- [[Notes/Planning/Demo Scope and Completion Plan|Demo scope and completion plan]]
- [[Notes/Planning/Project Audit and Backlog|Project audit and backlog]]
- [[Notes/Planning/Decision Log|Decision log]]

## Current focus

- [ ] Complete the short Burning Plains loop with Fire Spirit.
- [ ] Finish Fire, Wind, Water, and Earth as the four demo spirits.
- [ ] Complete Burning Plains and Earth Plains as the two demo areas.
- [ ] Validate menu, saving, victory, retry, performance, and controller flow.

See [[Notes/Planning/Demo Scope and Completion Plan|Demo scope and completion plan]] for the demo requirements and [[Notes/Planning/Project Audit and Backlog|Project audit and backlog]] for the wider technical backlog.

## Working habits

- Use **Open today's daily note** for each development session.
- Capture unprocessed ideas in [[Notes/Inbox/README|Inbox]].
- Record choices that affect scope or architecture in [[Notes/Planning/Decision Log]].
- Treat Unity assets and scripts as implementation truth; use this vault for intent, planning, and decisions.

## Source material

- `World of Spirits.docx` — original design document.
- `World Of Spirits/Assets/Docs/PROJECT_STRUCTURE.md` — current Unity organization.
- `World Of Spirits/Assets/Docs/SPIRIT_ABILITY_SETUP.md` — current ability component setup.
